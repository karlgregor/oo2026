function average_of_3 (number1: number, number2: number, number3: number): number {
    return (number1 + number2 + number3) / 3
}

console.log(average_of_3(100, 1529, 200))

function average_of_array (numbers: number[]): number {
    let sum = 0
...
function sliding_average_1 (numbers: number[], window_size = 2) : number[] {
    const out: number [] = []

    for (let i = 0; i < numbers.length - window_size + 1; i++) {
